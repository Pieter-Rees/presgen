    ...
    <div className="max-w-7xl mx-auto">
      <div className="text-center mb-12 relative">
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-4">
          <div className="bg-blue-100 text-blue-700 rounded-full text-xs px-8 py-1">
SAVED RECIPIENTS
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-2xl p-6 mb-8">
        <div className="bg-green-100 text-green-700 rounded-full text-xs px-3 py-1 mb-4 inline-block">SEARCH & FILTER</div>
        <div className="grid md:grid-cols-3 gap-4">
          <div className="col-span-2">
            <label className="text-sm font-bold text-indigo-600 uppercase tracking-wide block mb-2">
              Search
            </label>
            <input
              type="text"
              ...
              className="w-full px-4 py-3 border-2 border-indigo-600/50 rounded-lg bg-slate-900/50 backdrop-blur-sm text-slate-600 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-600 transition-all duration-300 font-semibold"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 border-2 border-indigo-600/50 rounded-lg bg-slate-900 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-600 transition-all duration-300 cursor-pointer font-semibold"
            >
              {relationships.map(relationship => (
                <option key={relationship} value={relationship} className="bg-slate-900 text-white">
      ...
      {filteredRecipients.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredRecipients.map((recipient) => (
            <div
              key={recipient.id}
              className="bg-white border border-slate-200 p-6 rounded-lg hover:shadow-2xl transition-all hover:scale-105"
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="text-xs text-slate-600 uppercase tracking-wide font-semibold">Recipient</p>
                  <h3 className="text-xl font-black text-indigo-600  uppercase">{recipient.name}</h3>
                  <p className="text-slate-600 capitalize font-semibold">{recipient.relationship}</p>
                </div>
                <span className="bg-indigo-100 text-indigo-700 rounded-full text-xs px-3 py-1">
                  {formatDate(recipient.savedAt)}
                </span>
              </div>

              <div className="space-y-3 text-sm text-slate-600 mb-4 font-semibold">
                <div className="flex justify-between">
                  <span>Age range</span>
                  <span className="text-indigo-600 font-bold">{formatAgeRange(recipient.age)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Budget</span>
                  <span className="text-indigo-600 font-bold">{getBudgetLabel(recipient.budget)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Occasion</span>
              ...
              <div className="bg-white border border-slate-200 border border-indigo-600 rounded-lg p-3 mb-4">
                <div className="bg-blue-100 text-blue-700 rounded-full text-xs px-2 py-0.5 mb-2 inline-block">TOP INTERESTS</div>
                <div className="flex flex-wrap gap-2">
                  ...
                  {recipient.interests.length > 3 && (
                    <span className="bg-green-100 text-green-700 rounded-full text-xs px-3 py-1">
              ...
              {recipient.additionalInfo && (
                <div className="text-sm text-slate-600 mb-4">
                  <div className="bg-green-100 text-green-700 rounded-full text-xs px-2 py-0.5 mb-1 inline-block">NOTES</div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-white border border-slate-200 text-indigo-600 py-2 px-3 rounded-lg text-sm font-bold uppercase tracking-wide hover:scale-105 transition-all duration-300 cursor-pointer"
                >
                  Use Profile
                </button>
                <button
                  onClick={() => onRemoveRecipient(recipient.id)}
                  className="bg-white border border-slate-200 border-2 border-red-600 text-red-400 py-2 px-3 rounded-lg text-sm font-bold uppercase tracking-wide hover:bg-red-600/20 transition-all duration-300 cursor-pointer"
                >
                  Remove
                </button>
              </div>
